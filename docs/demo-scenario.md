...
ライブレポートは最大 5 分間キャッシュされた結果を表示する場合があるため、障害注入後と復旧後に **再読み込み** を選択するようにしてください。
...
[定期タスク手順](sre-agent-setup.md#手順-5-毎朝-9-時-jst-の定期タスクを作成)でコスト用 API 権限と、変更履歴用の任意の Activity Log エクスポートを確認します。
